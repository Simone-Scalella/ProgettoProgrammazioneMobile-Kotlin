...
class SelfFragmentSuddLavoro : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_self_sudd_lavoro, container, false)
    }
    ...
    fun risultato(api: ApiRequests, token: String){
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val response = api.SelfSuddLavRequest(impostazioni,"Bearer "+token)
            // se la risposta è positiva viene inizializzata la recycleView
                if (response.isSuccessful) {
                    Log.d("mainActivityPro","va bene3")
                    var data = response.body()!!
                    withContext(Dispatchers.Main){
                        val recyclerView = view?.findViewById<RecyclerView>(R.id.recycler_view_simo_selfSuddLavoro)
                        recyclerView?.layoutManager = LinearLayoutManager(activity)
                        recyclerView?.adapter= CustomAdapterSuddivisioneLavoroSelf(data)
