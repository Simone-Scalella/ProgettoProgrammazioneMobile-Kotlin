...
import homeProf.suddivisione_lavoro.CustomAdapterSuddivisioneLavoroSelf
...
class SelfRichiesteLavoroFragment : Fragment() {

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        // Inflate the layout for this fragment
        return inflater.inflate(R.layout.fragment_self_richieste_lavoro, container, false)
    }
    ...
    fun risultato(api: ApiRequests, token: String){
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val response = api.SelfRichiesteRequest(impostazioni,"Bearer "+token)
            //se la risposta è positiva vado a inizializzare la recyclerView con la risposta del server
                if (response.isSuccessful) {
                    Log.d("mainActivityPro","va bene3")
                    var data = response.body()!!
                    withContext(Dispatchers.Main){
                        val recyclerView = view?.findViewById<RecyclerView>(R.id.recycler_view_simo_SelfRichieste)
                        recyclerView?.layoutManager = LinearLayoutManager(activity)
                        recyclerView?.adapter= CustomAdapterSelfRichieste(data,token)
